...
`traefik/` fronts every routed hostname (`lab53`, alongside AdGuard/cloudflared).
`traefik/dynamic/*.yml` uses Traefik's own Go-template `{{ env "VAR" }}` for anything that
shouldn't be a committed literal (domain, LAN CIDR, backend IPs) — Komodo's `[[VAR]]` only
templates the `environment` block, not arbitrary files. Unverified until a real deploy; check
the container log for the rendered value.

Real control for "LAN-only" is having no cloudflared Public Hostname / no AdGuard DNS Rewrite
for that hostname — both must point at `lab53`, never a backend origin directly. `lan-only`
(IP allowlist) is defense in depth on top, not the thing actually enforcing it.

One wildcard cert (`*.$DOMAIN`) at the entrypoint, reused by every router — per-router default
certs would leak LAN-only hostnames to public CT logs (crt.sh).
